    ...
    def forward(
        ...
        if self.sr_ratio > 1:
            ...
            if prompt_token_len != 0:
                prompt_token = hidden_states[:, :prompt_token_len, :]
            ...
            if prompt_token_len != 0:
                hidden_states = torch.cat((prompt_token, hidden_states), dim=1)
...
class SegformerAttention(nn.Module):
    ...
    def forward(self, hidden_states, height, width, prompt_token_len=0, output_attentions=False):
        self_outputs = self.self(hidden_states, height, width, prompt_token_len, output_attentions)
            ...
            for i, blk in enumerate(block_layer):
                if self.prompt_tokens[idx] is not None:
                    if len(self.prompt_tokens[idx].shape) == 3:
                        ...
                        this_layer_prompt_tokens = self.prompt_tokens[idx].unsqueeze(0)
