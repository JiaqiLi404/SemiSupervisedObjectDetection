...
class SegFormerModel(nn.Module):
    def __init__(self, pretrain_weight=None, lr=None, weight_decay=None, scheduler=None, device="cuda:0", *args, **kwargs):
        ...
        self.optimizer = torch.optim.Adam(filter(lambda x: x.requires_grad is not False, self.model.parameters()),
                                          ...
                                          betas=(0.5, 0.999))
        self.scheduler = torch.optim.lr_scheduler.ExponentialLR(self.optimizer, gamma=scheduler) #config.ModelConfig['scheduler']
    ...
    def eval_one_epoch(self, imgs, masks):
        self.model.eval()
        with torch.no_grad():  
            outputs = self.model(pixel_values=imgs, labels=masks)# logits are of shape (batch_size, num_labels, height/4, width/4)
            logits = outputs.logits.cpu()
    ...
    def train_one_epoch(self, imgs, masks):
        ...
        _loss = self.loss_function(predict_masks.to(self.device), masks)
